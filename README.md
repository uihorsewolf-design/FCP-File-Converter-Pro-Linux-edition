...
File Converter Pro is a Linux desktop application for converting image, video, audio and PDF files locally on your device. Conversion and transcription run locally and do not upload your files to an online service. Internet access is only used for the optional GitHub update check.
...
- Check the current FCP GitHub Releases page for available Linux updates
...
5. To build the project and generate the Linux installers locally:

```bash
npm run dist
```

The generated AppImage and Debian package are written to the `release/` folder. Linux builds use the Whisper engine bundled by `whisper-node`, so end users do not need a separate Whisper installation or internet access for transcription.

### Download the Linux app

End users do not need Node.js or the project source code. Download the latest AppImage or Debian package from the [FCP GitHub Releases](https://github.com/uihorsewolf-design/FCP-File-Converter-Pro-Linux-edition/releases) page.

### Build from source

Developers can install the dependencies and build new Linux installers locally with `npm run dist:linux`. The Linux Whisper engine is provided by the `whisper-node` dependency and is unpacked into the application at build time.
...
When an output folder is configured, individual downloads and ZIP archives are saved there automatically without opening a save dialog. Without an output folder, the Linux desktop environment asks where each download should be saved.
...
The vault stores each ZIP as an encrypted `.fcpv` container in a hidden `.fcp-vault` folder under the configured output path. File contents and new ZIP filenames are encrypted with AES-256-GCM. Linux file managers can see the container files but cannot read or decrypt them.
...
The password is never saved by the app. Losing the password means the encrypted files cannot be recovered. The quota limits vault contents but does not pre-allocate free disk space.
